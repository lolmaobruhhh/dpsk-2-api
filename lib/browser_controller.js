...
async function startVNCStack() {
    console.log('[VNC] Starting Xvfb + x11vnc + websockify...');
    stopVNCStack(); // Ensure clean slate synchronously

    // Start Xvfb
    const xvfb = spawn('Xvfb', [':99', '-screen', '0', '1280x720x24'], { shell: true });
    ...
    const vnc = spawn('x11vnc', ['-display', ':99', '-forever', '-shared', '-nopw', '-rfbport', '5900', '-noxdamage', '-defer', '5', '-ncache', '10', '-noshm'], { shell: true });
